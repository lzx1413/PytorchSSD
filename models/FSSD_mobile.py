...
def feature_transform_module(size):
    ...
    layers = []
    # conv4_3
    layers += [BasicConv(256, 256, kernel_size=1, padding=0)]
    # fc_7
...
class FSSD(nn.Module):
    ...
    def init_model(self, base_model_path):
        state_dict = torch.load(base_model_path, map_location=lambda storage, loc: storage)
